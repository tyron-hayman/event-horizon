...
<template>
  <div class="w-full px-5 py-20 md:px-10 xl:px-0 md:pt-60 md:pb-20 flex justify-center">
    <div class="container min-h-[500px]">
      <motion.h2
        class="text-white text-6xl md:text-8xl font-normal block pb-20 px-0 md:px-20 md:pb-20 text-center w-full md:w-2/3 !mx-auto"
        :variants="variants"
        initial="hidden"
        whileInView="visible"
      ...
      <div v-if="data">
        <div class="lg:grid lg:grid-cols-2 lg:gap-10">
          <motion.div
            ...
            :key="`test${index}`"
            class="bg-black border-white/5 border-1 border-solid rounded-3xl p-10 w-full md:w-auto !mb-10 lg:!mb-0 opacity-0 block"
          ...
          >
            <div class="flex items-center">
              <div
                class="!bg-cover w-[75px] h-[75px] rounded-full grayscale !mr-5 border-1 border-white/5 border-solid"
                :style="{ background: `url(${exp.Image.asset.url}) center center no-repeat` }"
              ></div>
              <h3 class="text-white text-4xl !font-black">{{ exp.name }}</h3>
            </div>
            <p class="text-[var(--text-color)] text-lg leading-loose py-5">{{ exp.content }}</p>
